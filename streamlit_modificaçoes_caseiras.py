...
def get_dynamic_paths():
    base_dir = os.getenv("BASE_DIR", os.getcwd())  # Usa o diretório atual como padrão
    samples_path = os.path.join(base_dir, "lab6", "dados de teste")
    save_directory = os.path.join(base_dir, "lab6", "dado de teste")
    image_path = os.path.join(base_dir, "lab6", "papicris.jpg")
    
    os.makedirs(save_directory, exist_ok=True)
    if not os.path.exists(samples_path):
        raise FileNotFoundError(f"Samples directory does not exist: {samples_path}")

    return samples_path, save_directory, image_path

samples_path, save_directory, image_path = get_dynamic_paths()

# Substituir imagens hardcoded por URLs ou caminhos configuráveis
background_image_url = os.getenv(
    "BACKGROUND_IMAGE_URL",
    "https://static.vecteezy.com/ti/vetor-gratis/p1/9482455-futebol-campo-futebol-campo-fundo-vetor.jpg"
)

### Arquivo: extractfilesfinal.py

# Caminhos dinâmicos para diretórios de dados

def get_data_directory():
    return os.getenv("DATA_DIRECTORY", "./dados")

directory = get_data_directory()

### Arquivo: Short_features.py

def get_feature_directory():
    return os.getenv("FEATURE_DIRECTORY", "./dados")

directory = get_feature_directory()

output_path = os.getenv("OUTPUT_PATH", "filtered_features.csv")  # Caminho dinâmico para salvar resultados
